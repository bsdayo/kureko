/** @type { import('../app/composables/kureko').KurekoConfig } */
// @ts-check

const config = {
  // 站点信息
  site: {
    // 站点名称
    name: 'Kureko Dev',
  },

  // 作者信息
  author: {
    // 名称
    name: 'Author',
    // 签名
    bio: 'The cake is a lie.',
    // 邮箱地址，用于联系作者
    email: 'someone@example.com',
    // 头像链接
    avatarUrl: 'https://placehold.co/256x256',

    // 社交链接。可以自由改变顺序、添加或删除
    social: [
      // 邮箱
      { type: 'email', link: 'mailto:someone@example.com' },
      // GitHub
      { type: 'github', link: 'https://github.com/octocat' },
      // Steam
      { type: 'steam', link: 'https://steamcommunity.com/id/xxxx' },
      // 哔哩哔哩
      { type: 'bilibili', link: 'https://space.bilibili.com/8047632' },
      // 网易云音乐
      { type: 'neteaseCloudMusic', link: 'https://music.163.com/#/artist?id=339594' },
      // 自定义
      {
        type: 'custom',
        // 图标 HTML 文本，优先级更高
        iconHtml:
          '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-party-popper-icon lucide-party-popper"><path d="M5.8 11.3 2 22l10.7-3.79"/><path d="M4 3h.01"/><path d="M22 8h.01"/><path d="M15 2h.01"/><path d="M22 20h.01"/><path d="m22 2-2.24.75a2.9 2.9 0 0 0-1.96 3.12c.1.86-.57 1.63-1.45 1.63h-.38c-.86 0-1.6.6-1.76 1.44L14 10"/><path d="m22 13-.82-.33c-.86-.34-1.82.2-1.98 1.11c-.11.7-.72 1.22-1.43 1.22H17"/><path d="m11 2 .33.82c.34.86-.2 1.82-1.11 1.98C9.52 4.9 9 5.52 9 6.23V7"/><path d="M11 13c1.93 1.93 2.83 4.17 2 5-.83.83-3.07-.07-5-2-1.93-1.93-2.83-4.17-2-5 .83-.83 3.07.07 5 2Z"/></svg>',
        // 图标链接
        iconUrl: 'https://placehold.co/32x32',
        // 目标链接
        link: 'https://example.com',
        // 图标颜色
        fg: '#fff',
        // 背景颜色
        bg: 'oklch(50% 0.134 242.749)',
      },
    ],
  },
  // 页脚信息
  footer: {
    // 年份，用于 Copyright 部分
    years: '2025',
    // 备案信息
    icp: {
      // 展示备案信息
      enabled: true,
      // 备案号
      text: '例ICP备12345678号',
      // 详情链接
      link: 'https://example.com/icp',
    },
  },
}

module.exports = config
